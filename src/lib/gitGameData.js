 ...
 * gitGameData.js
 * 
 * All static data for the Git Master Quest game:
 * - TIERS: The 3 learning tiers (Foundation / Branching / Advanced)
 * - CHEATSHEET: Categorized Git command reference
 * - QUICK_ACTIONS: Per-level suggested commands
 * - GAME_LEVELS: All 18 level definitions with initialState and goalCondition
 */

import { createInitialState } from './gitEngine'
...
export const CHEATSHEET = [
  {
    ...
    commands: [
      ...
      { cmd: 'git config --global user.email "e@x.com"',  desc: 'Set author email' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'git commit --amend',      desc: 'Fix last commit' },
...
export const QUICK_ACTIONS = {
  ...
  2:  ['git config --global user.name "Dev"', 'git config --global user.email "dev@ex.com"', 'git config --list'],
  ...
  6:  ['git diff', 'git diff --staged', 'git add index.html'],
  ...
  13: ['git tag v1.0.0', 'git tag -a v1.0.0 -m "Release"', 'git tag'],
...
export const GAME_LEVELS = [
  // ── TIER 1: Foundation ──────────────────────────────────────────────────
  {
    ...
    hint: '`git init` creates a `.git/` directory with all the plumbing. Your files aren\'t tracked yet — you still need to `add` and `commit` them.',
    initialState: {
      initialized: false,
  ...
  },
  {
    ...
    description: 'Every commit permanently records who made it. Before your first commit, you must tell Git your name and email — these are baked into every commit forever.',
    ...
    hint: '`--global` applies to all repos on your machine. Your identity is stored in `~/.gitconfig` and stamped on every commit forever.',
  ...
  },
  {
    ...
    description: 'Git uses a two-step commit process. First you STAGE the files you want to include, then you commit. The staging area lets you craft each snapshot precisely.',
    ...
    hint: 'After `git add`, run `git status` — staged files appear green under "Changes to be committed". Only staged files go into the next commit.',
    initialState: createInitialState({
      initialized: true,
      commits: {}, branches: { 'main': null }, head: 'main',
      workingDirectory: { 'index.html': 'untracked', 'styles.css': 'untracked' },
  ...
  },
  {
    ...
    description: 'A commit permanently saves your staged changes as a node in the history graph. Each commit has a unique hash, a message, author info, and a pointer to its parent.',
    objective: 'Commit your staged changes with a descriptive message.',
    instructions: 'Run `git commit -m "Initial commit"`. All staged files will be recorded as the first commit.',
    hint: 'Write commit messages in imperative mood: "Add feature", not "Added feature". Run `git log` afterward to see your commit in history.',
    initialState: createInitialState({
      initialized: true,
      commits: {}, branches: { 'main': null }, head: 'main',
  ...
  },
  {
    ...
    description: '`git log` shows the chain of commits — who changed what, when, and why. `git show` lets you inspect a single commit\'s full diff in detail.',
    objective: 'Use `git log` or `git show` to explore the commit history.',
    instructions: 'Run `git log` to see all commits. Try `git log --oneline` for a compact view. Then run `git show C1` to inspect a specific commit.',
    hint: '`git log` follows the parent chain backward from HEAD. Use `--oneline` for a quick overview. `git show C1` prints the commit diff and metadata.',
    initialState: createInitialState({
      initialized: true,
      commits: {
  ...
  },
  {
    ...
    instructions: 'Run `git diff` to see unstaged changes. Run `git diff --staged` to see what\'s already staged. Then stage files with `git add index.html`.',
    hint: '`git diff` compares working dir vs staging area. `git diff --staged` compares staging area vs last commit.',
    initialState: createInitialState({
      initialized: true,
      commits: {
  ...
  // ── TIER 2: Branching ────────────────────────────────────────────────────
  {
    ...
    description: 'Branches are lightweight movable pointers to commits. They allow you to work in isolation without affecting the main codebase. Creating a branch is essentially free.',
    objective: 'Create a new `dev` branch and switch to it.',
    instructions: 'Run `git branch dev` to create the branch, then `git switch dev` to switch. Or use `git switch -c dev` to do both at once.',
    hint: 'A branch is just a file containing a commit hash. HEAD is a pointer that moves with you as you switch branches.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',       branch: 'main' },
  ...
  },
  {
    ...
    description: 'When the branch you\'re merging is directly ahead (no divergence), Git fast-forwards: it simply moves the branch pointer forward. No merge commit is created — history stays linear.',
    ...
    hint: 'Fast-forward is possible because `feature` is a direct descendant of `main` — a straight line from C1 → C2. Use `--no-ff` to force a merge commit.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',       branch: 'main' },
  ...
  },
  {
    ...
    description: 'When both branches have diverged, Git performs a three-way merge using the common ancestor. This creates a new merge commit with TWO parents — the graph shows a diamond shape.',
    objective: 'Merge the `dev` branch into `main` (creates a merge commit).',
    instructions: 'You\'re on `main`. Both branches diverged from C0. Run `git merge dev` to create a merge commit.',
    hint: 'Git finds common ancestor C0, then combines changes from C1 (main) and C2 (dev) into a new merge commit C3 with two parents.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',     branch: 'main' },
  ...
  },
  {
    ...
    description: 'Rebasing moves commits to a new base, replaying them one-by-one. The result is a clean linear history — as if you\'d always branched from the latest commit.',
    objective: 'Rebase the `feature` branch onto `main`.',
    instructions: 'You\'re on `feature`. Run `git rebase main` to replay the feature commits on top of C1.',
    hint: 'Rebase creates NEW commits (new hashes). Golden rule: NEVER rebase commits already on a shared remote branch — it rewrites history others depend on.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',      branch: 'main' },
  ...
  },
  {
    ...
    description: 'Cherry-pick copies a specific commit from anywhere in history and applies it on top of your current branch. A new commit is created with the same changes but a new hash.',
    objective: 'Cherry-pick the bugfix commit `C2` from `dev` onto `main`.',
    instructions: 'You\'re on `main`. Run `git cherry-pick C2` to copy the critical bugfix from the dev branch.',
    hint: 'Cherry-picking is useful when you need one specific fix from another branch without merging everything else.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',             branch: 'main' },
  ...
  },
  {
    ...
    objective: 'Stash your work-in-progress, then restore it with `git stash pop`.',
    instructions: 'Run `git stash` to save changes. Run `git stash list` to see the stash stack. Run `git stash pop` to restore them.',
    hint: 'Stash is a stack — you can push multiple entries. `pop` restores and removes; `apply` restores without removing.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit', branch: 'main' },
    ...
    goalCondition: (_, cmds) => cmds.has('stash')
  },
  {
    id: 13, tier: 2,
    title: '13. Tagging Releases (git tag)',
    description: 'Tags are permanent markers in history — used for release versions (v1.0.0). Unlike branches, tags never move. Annotated tags store a message, tagger, and timestamp.',
    objective: 'Create an annotated tag `v1.0.0` marking this commit as a stable release.',
    instructions: 'Run `git tag -a v1.0.0 -m "First stable release"`. Then run `git tag` to list all tags.',
    hint: 'Always use annotated tags for releases (`-a` flag) — they carry full metadata and can be GPG-signed.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',     branch: 'main' },
  ...
  // ── TIER 3: Advanced & Remote ────────────────────────────────────────────
  {
    ...
    description: 'A "remote" is a named reference to another Git repository — typically on GitHub or GitLab. By convention the primary remote is called `origin`. Remotes tell Git where to push and pull from.',
    ...
    hint: 'You can have multiple remotes (e.g., `origin` for your fork, `upstream` for the original).',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',         branch: 'main' },
  ...
  },
  {
    ...
    description: 'Pushing uploads your local commits to the remote repository. The first push uses `-u` to set the upstream tracking branch. After that, `git push` alone is enough.',
    objective: 'Push your local `main` branch to the remote origin.',
    instructions: 'Check status with `git status`, then run `git push -u origin main` to push all commits.',
    hint: '`-u` (or `--set-upstream`) permanently links your local branch to `origin/main`. Git remembers where to push without specifying the remote each time.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',         branch: 'main' },
  ...
  },
  {
    ...
    description: '`git fetch` downloads new commits from the remote but does NOT merge them. `git pull` does fetch + merge in one step. In team environments, prefer fetch → review → merge.',
    objective: 'Pull the latest changes from the remote to update your local `main` branch.',
    instructions: 'The remote is ahead by 1 commit (C1). Run `git fetch` to download, then `git pull` to integrate. Or just `git pull` directly.',
    hint: '`git fetch` updates `origin/main` but leaves local `main` unchanged. `git pull` is `git fetch` + `git merge`.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',                    branch: 'main' },
  ...
  },
  {
    ...
    description: '`git reset` moves the branch pointer backward. `--soft` keeps changes staged, `--mixed` unstages them, `--hard` discards them permanently. Only use on LOCAL commits.',
    objective: 'Discard the broken commit C2 and reset `main` back to C1.',
    instructions: 'Run `git log --oneline` to see history. Then run `git reset --hard HEAD~1` to discard the last commit.',
    hint: '`HEAD~1` = one commit before HEAD. `--hard` discards both the commit AND its working directory changes. Never use `reset` on commits already pushed to a shared branch.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',          branch: 'main' },
  ...
  },
  {
    ...
    description: '`git revert` is the SAFE way to undo on shared branches. It creates a NEW commit that reverses the changes, preserving history completely — safe for team use.',
    ...
    hint: '`git revert` never deletes history — it adds an "undo" commit. Rule: if commits are already pushed → use `revert`. Local only → `reset` is fine.',
    initialState: createInitialState({
      initialized: true,
      commits: {
        'C0': { id: 'C0', parents: [],     message: 'Initial commit',               branch: 'main' },
